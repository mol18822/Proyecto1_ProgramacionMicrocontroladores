...
; ------ Para PB1, encargado de seleccionar el semáforo a configurar --------- ;
    ...
    btfss ((STATUS) and 07Fh), 2 ; si se vuelve a presionar el PB
    goto $+3 ;
...
Min_Time:
    ...
    bcf ((INTCON) and 07Fh), 0 ; ((INTCON) and 07Fh), 0 == 0
    return
...
oscillator:
    BANKSEL TRISA
    bsf ((OSCCON) and 07Fh), 6 ; 1
    bsf ((OSCCON) and 07Fh), 5 ; 1
    bcf ((OSCCON) and 07Fh), 4 ; 0 4 MHz
    bsf ((OSCCON) and 07Fh), 0 ; Activar reloj interno
    return
...
; ---- Push bottons de control y leds que indican el semáforo a configurar --- ;
    ...
    bsf TRISB, PB1_Estado ; ((PORTB) and 07Fh), 1 como input para PB1
    bsf TRISB, PB2_Increase ; ((PORTB) and 07Fh), 2 como input para PB2
    bsf TRISB, PB3_Decrease ; ((PORTB) and 07Fh), 3 como input para PB3
...
config_Timer0:
    BANKSEL TRISA ;
    bcf ((OPTION_REG) and 07Fh), 5 ; Clock interno == 1
    bcf ((OPTION_REG) and 07Fh), 3 ; Preescaler == 1
    bsf ((OPTION_REG) and 07Fh), 2 ; 1
    bsf ((OPTION_REG) and 07Fh), 1 ; 1
    bcf ((OPTION_REG) and 07Fh), 0 ; 0 --> 1:128
    return
...
Decenas10:
    ...
    subwf Divisor10, F ;
    btfsc ((STATUS) and 07Fh), 0 ; Carry == 0 --> 10 > Divisor10
    incf Decenas ;
